...
export default function PendingServices() {
  return <Redirect href="/assigned-services" />;
}

/*
ORIGINAL PENDING SERVICES IMPLEMENTATION (COMMENTED OUT FOR PRESERVATION)
...
export function DisabledPendingServices() {
    ...
    const updatedBookings = await Promise.all(
      (bookings || []).map(async (booking: any) => {
        ...
        if (serviceId) {
          ...
          totalStaffAmount = Number(String(rawAmount).replace(/[^0-9.]/g, ""));
        }

        return {
          ...booking,
          total_staff_amount: totalStaffAmount,
        };
      }),
    );
  ...
  const updateResponse = async (
    ...
    if (!error && status === "APPROVED") {
      try {
        const booking = services.find((s) => s.id === id);
        ...
        if (booking && staff) {
          await supabase.functions.invoke("send-booking-assignment", {
            body: {
              customer_phone: booking.phone_number,
              customer_name: booking.customer_name,
              service_name: booking.services?.[0]?.title || "Service",
              booking_date: `${booking.booking_date} ${booking.booking_time}`,
              professional_name: staff.name,
              professional_phone: staff.phone,
            },
          });
