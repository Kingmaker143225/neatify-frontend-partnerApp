...
export default function ZoneMapScreen() {
  ...
  try {
    // =====================================================
    // 1. GET ASSIGNED ZONE FROM BACKEND
    // =====================================================
...
if (!zoneData) {
  setDataError(
    "Unable to fetch assigned zone. Please try again."
  );
  return;
}

    const hubName =
      zoneData.hub_name || "Assigned Zone";
    ...
    try {
      ...
      if (status === "granted") {
        ...
        if (
          loc?.coords &&
          isFiniteCoord(loc.coords.latitude) &&
          isFiniteCoord(loc.coords.longitude)
        ) {
          locCoords = {
            latitude: loc.coords.latitude,
            longitude: loc.coords.longitude,
      ...
      console.log(
        "Location fetch error in zone-map:",
        locErr
      );
    }

    // =====================================================
    // 3. FALLBACK TO BACKEND STORED LOCATION
    // =====================================================

    if (!locCoords && zoneData.live_location) {
      const parts = String(
        zoneData.live_location
      )
        .split(",")
        .map((s: string) =>
          parseFloat(s.trim())
      ...
      if (
        parts.length === 2 &&
        isFiniteCoord(parts[0]) &&
        isFiniteCoord(parts[1])
      ) {
        locCoords = {
          latitude: parts[0],
          longitude: parts[1],
    ...
    // =====================================================
    // 4. ASSIGNED LOCATIONS FROM BACKEND
    // =====================================================

    const rawSubLocs: Array<{
      location_name: string;
      pincode: string;
    }> = (
      zoneData.sub_locations || []
    ).map((item: any) => ({
      location_name: String(
        item.location_name || ""
      ).trim(),
    ...
    if (rawSubLocs.length > 0) {
      const formatted = rawSubLocs
        .map((s) =>
          s.pincode
            ? `${s.location_name} (${s.pincode})`
            : s.location_name
        )
        .join(" • ");

      setAssignedLocationsStr(
        formatted
      );
    } else {
      setAssignedLocationsStr(
    ...
    // =====================================================
    // 5. GEOCODE SUB LOCATIONS
    // =====================================================

    const geocodedSubLocs: HubSubLocation[] =
      await Promise.all(
        rawSubLocs.map(async (sub) => {
          let lat: number | undefined;
          let lng: number | undefined;
    ...
    // =====================================================
    // 6. COMPUTE HUB CENTROID
    // =====================================================

    const validCoords =
      geocodedSubLocs.filter(
        (s) =>
          isFiniteCoord(s.latitude) &&
          isFiniteCoord(s.longitude)
      );
    ...
    if (validCoords.length > 0) {
      ...
      try {
        ...
        if (
          ...
          mainHubGeo.length > 0 &&
          isFiniteCoord(
            mainHubGeo[0].latitude
          ) &&
          isFiniteCoord(
            mainHubGeo[0].longitude
          )
        ) {
          hubCentroid = {
            latitude:
    ...
    // =====================================================
    // 7. FILL MISSING SUB LOCATION COORDINATES
    // =====================================================

    geocodedSubLocs.forEach(
      (sub, idx) => {
        if (
          !isFiniteCoord(sub.latitude) ||
          !isFiniteCoord(sub.longitude)
        ) {
          const angle =
            (idx * 2 * Math.PI) /
            Math.max(
              1,
              geocodedSubLocs.length
            );
    ...
    if (
      !liveInside &&
      locCoords &&
      geocodedSubLocs.length > 0
    ) {
      for (
        const sub of geocodedSubLocs
      ) {
        if (
          isFiniteCoord(
            sub.latitude
    ...
    // =====================================================
    // 9. UPDATE STATUS THROUGH BACKEND
    // =====================================================
    ...
    try {
      ...
      console.log(
        "❌ Failed to update partner location:",
        error
      );
    }
  } else {
    console.log(
      "⚠️ Location unavailable. Skipping location update."
    );
  }
  } catch (err) {
    console.log(
